...
export class DateUtil {
  ...
  find(list: any, findFunc: any, startIndex?: any, endIndex?: any) {
    const f = this.getFindFunc(findFunc);
    const e = this.getFindIndex(list, endIndex, list.length);
    let r: any;
  ...
  parseDate(date: string, fmt: string) {
    const indexMap: any = {}; // contains reGroupPosition -> typeLetter or [typeLetter, value array]
    let reIndex = 1;
      ...
      format.replace(
        ...
        (wholeMatch, placeholderChar, placeholderDigits, param) => {
          if (/[dmhkyhs]/i.test(placeholderChar)) {
            ...
            const plen = placeholderDigits.length + 1;
    ...
    const ctorArgs = [0, 0, 0, 0, 0, 0, 0];
    for (let i = 1; i < reIndex; i++) {
      const matchVal = match[i];
      const indexEntry: any = indexMap[i];
      if (this.isList(indexEntry)) {
        // for a, n or N
        const placeholderChar = indexEntry[0];
        const mapEntry = this.parseDateMap[placeholderChar];
        const ctorIndex = mapEntry[0];
        const valList = indexEntry[1] || mapEntry[1];
        const listValue = this.find(valList, (v: any, index: any) => {
        ...
        const value = parseFloat(matchVal);
        const mapEntry = this.parseDateMap[indexEntry];
  ...
  isSameHour(d1: Date, d2: Date) {
    return (
      d1 && d2 && d1.getHours() === d2.getHours() && this.isSameDay(d1, d2)
    );
  }
  ...
  isInPreviousMonth(startDate: Date, endDate: Date) {
    const previousMonth = this.getDateInPreviousMonth(startDate);
  ...
  incrementMonths(date: Date, numberOfMonths: number) {
    ...
    const dateInTargetMonth = new Date(
      date.getFullYear(),
  ...
  isDateWithinRange(date: Date, minDate: Date, maxDate: Date) {
    const dateAtMidnight = this.createDateAtMidnight(date);
    const minDateAtMidnight = this.isValidDate(minDate)
      ? this.createDateAtMidnight(minDate)
      : null;
    const maxDateAtMidnight = this.isValidDate(maxDate)
