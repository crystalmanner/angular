...
export class DateLocale {
  ...
  getMonthNames(style: 'long' | 'short' | 'narrow'): string[] {
    ...
    if (SUPPORTS_INTL_API) {
      ...
      return range(12, i =>
        this._stripDirectionalityCharacters(dtf.format(new Date(2017, i, 1)))
      );
    }
  ...
  getHourNames(): string[] {
    ...
    if (SUPPORTS_INTL_API) {
      const dtf = new Intl.DateTimeFormat(this.locale, { hour: 'numeric' });
      return range(24, i =>
        this._stripDirectionalityCharacters(dtf.format(new Date(2017, 0, 0, i)))
      );
    }
  ...
  getMinuteNames(): string[] {
    ...
    if (SUPPORTS_INTL_API) {
      const dtf = new Intl.DateTimeFormat(this.locale, { minute: 'numeric' });
      return range(60, i =>
        this._stripDirectionalityCharacters(
  ...
  getDateLabel(d: Date): string {
    const day: string = this.getDayOfWeekNames('short')[d.getDay()];
    const date: string = this.getDateNames()[d.getDate() - 1];
    const month: string = this.getMonthNames('short')[d.getMonth()];
    return `${day}, ${month} ${date}`;
  }

  getHoursLabel(d: Date): string {
    return `${this.getHourNames()[d.getHours()]}`;
  }

  getMinutesLabel(d: Date): string {
    return `${this.getMinuteNames()[d.getMinutes()]}`;
  }
