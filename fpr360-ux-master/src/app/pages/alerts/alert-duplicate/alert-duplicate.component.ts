...
export class AlertDuplicateComponent implements OnInit {
 ...
 splittedArray:any[];
  ...
  constructor(public dialog: MatDialog,private route :ActivatedRoute,private router:Router, private http:Http,private alReasonService: AlertReasonService) {
 route.queryParams.subscribe(
  ...
  data => {let testdata = data['param'];
    ...
    //this.dataArray = this.passedData.split(',');
     ...
     this.router.navigate(['/alerts/alert-duplicate']);
     this.stayHere();
  })
...
public postAlert(id,outcome,reason,notes,refuAmount,refuTransId,saleTransId):Observable<AlertResolveApi>{
  ...
  let alertUpdate = new AlertResolveApi(id,"","test1","","","",time,time,"true","vv","","","","","","",saleTransId,"",refuTransId,outcome,reason,refuAmount,notes,"Resolved");
